...
Pathfinder.prototype = {
	...
	,handleGetFullWorldInfo: function(data) {
		...
		while(_g < _g1) {
			...
			while(_g2 < _g3) {
				...
				while(_g4 < _g5) {
					...
					if(buildingID != -1) {
						...
						var isLeftInaccessible = (buildingType & 2) > 0;
						var isRightInaccessible = (buildingType & 4) > 0;
						buildingType >>= 4;
						var thisBuilding = new PathfindingBuilding(buildingID,continuousBuildingID++,buildingType,b * 20,w,bl,isRooftopBuilding,isLeftInaccessible,isRightInaccessible);
						if(bl == 0) {
			...
			while(_g6 < _g7) {
				...
				while(_g8 < _g9) {
					...
					var thisIsLeftInaccessible = buildingArray[x][y].isLeftInaccessible;
					var thisIsRightInaccessible = buildingArray[x][y].isRightInaccessible;
					var thisIsRooftopBuilding = buildingArray[x][y].isRooftopBuilding;
					if(x > 0 && y > 0 && buildingArray[x - 1].length > y && !thisIsLeftInaccessible && buildingArray[x - 1][y] != null && !buildingArray[x - 1][y].isRightInaccessible) {
						var prio = 20;
						if(thisBuilding1.type == 2 || buildingArray[x - 1][y].type == 2) {
							prio *= 3;
						}
						walkThroughEdges.push(new PathfindingEdge(2,buildingArray[x - 1][y],prio));
					}
					if(x < numberOfBuildingArrays - 1 && y > 0 && buildingArray[x + 1].length > y && !thisIsRightInaccessible && buildingArray[x + 1][y] != null && !buildingArray[x + 1][y].isLeftInaccessible) {
						var prio1 = 20;
						if(thisBuilding1.type == 2 || buildingArray[x + 1][y].type == 2) {
							prio1 *= 3;
						}
						walkThroughEdges.push(new PathfindingEdge(3,buildingArray[x + 1][y],prio1));
					}
					if(y < buildingArray[x].length - 1 && buildingArray[x][y + 1] != null && !thisIsRooftopBuilding) {
		...
		while(_g < _g1) {
			...
			var extraPrio = 1;
			if(thisBuilding.type == 2 || otherBuilding.type == 2) {
				extraPrio *= 3;
			}
			this.worlds[thisBuilding.world][thisBuilding.x / 20 | 0][thisBuilding.yIndex].connections.push([new PathfindingEdge(connectionType,otherBuilding,data[i++] * extraPrio)]);
		}
...
var PathfindingBuilding = function(id,continuousID,type,x,world,yIndex,isRooftopBuilding,isLeftInaccessible,isRightInaccessible) {
	this.connections = [];
	this.id = id;
	...
	this.isLeftInaccessible = isLeftInaccessible;
	this.isRightInaccessible = isRightInaccessible;
};
